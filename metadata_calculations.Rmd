...
# For each mean weight, go through all rows and look for samples with the same weight.
...
s <- seq(1, maxWeight, by = 0.5) 
for (j in s) {
  # For each weight, go through all rows and look for samples with the same weight. Since it is impossible to go over decimals in a *for-loop*, I've added a vector 's' that is able to do this which is then called upon in the *for-loop*.
